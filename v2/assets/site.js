/* v2 site.js — loads JSON catalogues and supports live search filter */
(function(){
  'use strict';
  ...
  function buildItem(item, i){
    ...
    return '<article class="entry js-filter-item" data-search="'+esc(search)+'">'+
      '<div class="entry-img"><span class="num">'+n+'</span><img src="'+esc(img(item.image))+'" alt="'+esc(item.title)+'"></div>'+
      '<div class="entry-body">'+
        '<div class="meta-small">— Nº '+n+'</div>'+
        '<h3>'+esc(item.title)+'</h3>'+
        '<p>'+nl(item.description)+'</p>'+
        (item.cost ? '<div class="cost"><small>Hire</small>'+nl(item.cost)+'</div>' : '')+
      '</div>'+
    '</article>';
  }
  ...
  function buildPersonal(item){
    if(item.type === 'intro'){
      return '<div class="intro-block js-filter-keep" style="grid-column:1/-1"><h2>A note on <em>'+esc(item.title)+'</em></h2><p>'+nl(item.description)+'</p></div>';
    }
    if(item.type === 'heading'){
      return '<div class="heading-rule js-filter-keep" style="grid-column:1/-1">'+esc(item.title)+'</div>';
    }
    var cls = item.size === 'full' ? 'p-card full js-filter-item' : 'p-card half js-filter-item';
    var search = ((item.title||'')+' '+(item.description||'')+' '+(item.category||'')+' '+(item.tags||'')).toLowerCase();
    return '<div class="'+cls+'" data-search="'+esc(search)+'">'+
      '<img src="'+esc(img(item.image))+'" alt="'+esc(item.title)+'">'+
  ...
  function wireSearch(container){
    ...
    input.addEventListener('input', function(){
      ...
      if(!anyVisible){
        ...
        div.style.cssText = 'grid-column:1/-1;text-align:center;padding:60px 0;color:var(--muted);font-family:Fraunces,serif;font-style:italic;font-size:20px';
  ...
  function load(container){
    ...
    fetch(src).then(function(r){return r.json();}).then(function(items){
      ...
      container.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:var(--muted);font-family:Fraunces,serif;font-style:italic">Unable to load items.</p>';
    });
  }
